...
def minimize(func, extra_args: dict, bounds, precision=1e-5, mach_eps=MACH_32BIT_EPS, maxiter=20, log_gold=False):
    """
    Vectorized implementation of Brent's minimization method, solving multiple functional minimization problems in a
    shared execution flow for vastly improved performance over iterative execution of individual problems. This function
    gets JIT compiled and requires recompilation according if func, maxiter, log_gold, or the dimensionality of
    extra_args is changed.

    Parameters
    ----------
    func: The function for which to find the value for x that minimizes f(x, ...), must be array computable.
    extra_args: Additional arrays of arguments required to compute f, different for each sub-problem. This argument
                defines the dimensionality of the problem.
    bounds: The allowable range of possible values for x, shared across all sub-problems.
    precision: Absolute minimum tolerance for convergence. Default is 1e-5.
    mach_eps: Machine precision used to compute the scaling relative tolerance for convergence. Default is 2**-24.
    maxiter: The maximum number of iterations before returning current estimates for each sub-problem x.
    log_gold: Whether to internally use x or log10(x), improves efficiency when working with exponential functions.

    Returns
    -------
    x_min: The value of x that minimizes f(x, ...).
    """
...
def brent_minimize_std(f, extra_args, bounds, maxiter=10, rel_tol=MACH_32BIT_EPS**0.5, abs_tol=1e-5):
    """
    Implementation of Brent's minimization method in base form, helps for understanding the vectorized version.
    ...
    Parameters
    ----------
    f: The function for which to find the value for x that minimizes f(x, ...).
    extra_args: Additional arguments required to compute f.
    bounds: The allowable range of possible values for x.
    maxiter: The maximum number of iterations before returning a not yet sufficiently precise estimate for x.
    rel_tol: Relative tolerance for convergence, the exact required precision is a function of both rel and abs tol.
    abs_tol: Absolute tolerance for convergence, the exact required precision is a function of both rel and abs tol.

    Returns
    -------
    x_min, f(x_min): The value of x that minimizes f(x, ...), alongside the minimized value.
    """
    # We divide by three so that abs_tol directly represents the furthest that the estimated minimum can be from the
    # true minimum
    ...
    while True:
        ...
        if fu <= fx:
            ...
            # If fu is our new second least point or if we are near the start of the algorithm and can't yet do
            # interpolation steps
            if fu <= fw or w == x:
                v, w, fv, fw = w, u, fw, fu
