...
def electromigration_qualification():
    ...
    mb.add_latent_variable('em_n', pymc.Normal, {'mu': 1.8, 'sigma': 0.1})
    mb.add_latent_variable('em_eaa', pymc.Normal, {'mu': 2, 'sigma': 0.2})
    mb.add_latent_variable('em_var', pymc.Normal, {'mu': 0.07, 'sigma': 0.04})

    mb.define_experiment_params(
        ['vdd', 'temp'], simultaneous_experiments=['t1'],
        samples_per_observation={'em_ttf': 5})
    ...
    estimate = tm.estimate_reliability(percent_bound, num_samples=30000)
    ...
    fail states.
    '''
    tm.set_experiment_conditions({'t1': {'vdd': selected_test['t1']['vdd'], 'temp': selected_test['t1']['vdd']}})
